...
class HRAssistant:
    ...
    def _construct_gemini_rag_prompt(
        ...
        retrieved_chunks: list[HRSearchResult],
    ) -> str:
        contents = []
    ...
    def chat(
    ...
    ) -> tuple[ConversationTurn, list[HRSearchResult]]:
        ...
        retrieved_knowledge = self.knowledge_base.search(retrieval_query)
...
if __name__ == "__main__":
    ...
    messages = [
        ConversationTurn(role="user", content="How often is the company policy reviewed and updated?"),
    ]
    response, _ = assistant.chat(messages)
    messages.append(response)
    logger.info(response.content)
    new_question = "Will employees be notified of major adjustments to policies?"
    messages.append(ConversationTurn(role="user", content=new_question))
    response, _ = assistant.chat(messages)
    messages.append(response)
    logger.info(response.content)
    new_question = "what is a pokemon?"
    messages.append(ConversationTurn(role="user", content=new_question))
    response, _ = assistant.chat(messages)
    messages.append(response)
    logger.info(response.content)